...
def create_borrow_v1(db: Session, borrow_in: BorrowRecordCreate, current_user_id: int):
    if not borrow_in.book_id and not borrow_in.collection_id:
        raise AppException(status_code=status.HTTP_400_BAD_REQUEST, error_code=ErrorCode.VALIDATION_ERROR, message="Phải cung cấp book_id hoặc collection_id")

    owner_id = None
    if borrow_in.book_id:
        book = db.query(Book).filter(Book.id == borrow_in.book_id).first()
        if not book: raise AppException(status_code=status.HTTP_404_NOT_FOUND, error_code=ErrorCode.ITEM_NOT_FOUND, message="Không thấy sách")
        owner_id = book.owner_id
    elif borrow_in.collection_id:
        coll = db.query(Collection).filter(Collection.id == borrow_in.collection_id).first()
        if not coll: raise AppException(status_code=status.HTTP_404_NOT_FOUND, error_code=ErrorCode.ITEM_NOT_FOUND, message="Không thấy Collection")
...
def get_borrow_by_id_v1(db: Session, borrow_id: int, current_user_id: int):
    ...
    if record.borrower_id != current_user_id and record.owner_id != current_user_id:
        raise AppException(status_code=status.HTTP_403_FORBIDDEN, error_code=ErrorCode.PERMISSION_DENIED, message="Không có quyền xem")
    return record
...
def update_borrow_status_v1(db: Session, borrow_id: int, status_update: BorrowRecordUpdateStatus, current_user_id: int):
    ...
    if record.owner_id != current_user_id:
        raise AppException(status_code=status.HTTP_403_FORBIDDEN, error_code=ErrorCode.PERMISSION_DENIED, message="Chỉ chủ sở hữu (Owner) mới có quyền đổi trạng thái duyệt đơn")
